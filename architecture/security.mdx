---
title: "Security Model: JWT, bcrypt, Rate Limiting, and Anti-Spoofing"
...
<Tip>
  The JWT payload contains `{ usuario_id, iat, exp }`. Controllers always read `req.usuario.usuario_id` — they never accept a user ID from `req.body` or `req.params` for operations that require ownership verification.
</Tip>
...
<CardGroup cols={2}>
  <Card title="File type filter" icon="file-shield">
    Only images (`image/jpeg`, `image/png`, `image/gif`, `image/webp`) are accepted. Other MIME types are rejected before the file is written to disk.
  </Card>
