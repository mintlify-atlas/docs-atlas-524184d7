...
Creates a new user account. On success, returns a signed JWT alongside the new user's public profile. The password is hashed with bcrypt (10 rounds) before being stored — the plain-text value never touches the database.
...
<CodeGroup>
```bash cURL
curl -X POST https://your-api.com/api/auth/reset-password \
  -H "Content-Type: application/json" \
  -d '{
