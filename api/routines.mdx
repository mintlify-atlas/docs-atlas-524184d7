...
<Note>
  Each user is limited to **4 active routines**. The count is checked at the moment of each creation request to eliminate race conditions between concurrent tabs or sessions. If the limit is already reached, the server returns `403 Forbidden` regardless of how the request was formed.
</Note>
